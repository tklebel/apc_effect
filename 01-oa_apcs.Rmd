...
papers <- spark_read_csv(sc, "/user/tklebel/fos/fos_papers_collated.csv",
                         name = "papers")
fos_labels <- spark_read_csv(sc, "/user/tklebel/fos/fos_papers_FOS_info.csv",
                             name = "fos_labels")
fos_labels <- fos_labels %>% 
  select(paperid, fieldofstudyid, fos_displayname)

fos_lookup <- spark_read_csv(sc, "/user/tklebel/fos/fos_lookup.csv",
                             name = "fos_lookup")
...
author_paper_affiliations <- spark_read_csv(
  sc,
  "/user/tklebel/fos/fos_author_paper_affil.csv",
...
affil_leiden_key <- read_csv2(
  here::here("data/matching_leiden.csv")
)
...
step1 <- papers %>% 
  ...
  filter(!is.na(journalid), is_oa, year < 2020,
         provider_cat != "Repository only") %>% 
  left_join(apc_info, by = "journalid") %>% 
...
step1_with_fos <- step1 %>% 
  select(year, paperid, APC) %>% 
  left_join(fos_labels) %>% 
  left_join(fos_lookup, by = c("fieldofstudyid" = "fos_id"))
  

apc_per_fos <- step1_with_fos %>% 
  mutate(APC = if_else(APC == "NA", "Not in DOAJ", APC)) %>% 
  group_by(top_fos, year) %>% 
  count(APC) %>% 
  collect()
```


```{r doaj-share, fig.height=7, fig.width=7}
p <- apc_per_fos %>% 
  filter(year %in% 2015:2018) %>%
  group_by(top_fos, APC) %>% 
  summarise(n = sum(n)) %>% 
  mutate(prop = n / sum(n)) %>% 
  mutate(APC = factor(APC, levels = c("Not in DOAJ", "FALSE", "TRUE"),
                      labels = c("Not in DOAJ", "No", "Yes"))) %>%
  ggplot(aes(prop, fct_reorder(top_fos, prop, max), fill = APC)) + # need the trick on reordering here, should be reordered by "Yes"
  geom_col(width = .7) +
