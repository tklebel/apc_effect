...
```{r}
average_over_fields <- function(model, orig_var, q = .5, 
  ...
  contrast_all_recomputed <- contrast_all %>% 
    mutate(x = factor(P_top10, labels = c("base", "step"))) %>% 
    pivot_wider(-c(predicted, conf.low, conf.high, P_top10, rowid), 
                names_from = x, values_from = draw) %>% 
    mutate(contrast = step / base - 1) %>% 
...
```{r}
scale_offset <- attributes(base$P_top10)[["scaled:center"]]
x1_identity <- quantile(df$P_top10, .2)
x2_identity <- x1_identity * 1.01
x1_log <- log(x1_identity) - scale_offset
x2_log <- log(x2_identity) - scale_offset
...
At 50%
```{r}
contrast_at_50 <- average_over_fields(hm, df$P_top10)
```
```{r fig.width=7, fig.height=8}
contrast_at_50 %>% 
  rename(effect = ame) %>% 
...
```{r}
contrast_at_80 <- average_over_fields(hm, orig_var =  df$P_top10,
                                      q = .8)
```
...
contrast_at_80 %>% 
  rename(effect = ame) %>% 
  plot_effect(location = "the 80% quantile")
```
...
all_joined <- bind_rows(
  rename(contrast_at_50, effect_50 = ame),
  rename(contrast_at_80, effect_80 = ame),
  rename(contrast_all_recomputed, effect_20 = ame)
)
```
...
p <- all_joined %>% 
  ...
  labs(
    y = NULL, 
    x = glue::glue("% Change of APC for 1% change of P_top10% at "),
    caption = "Averaged predictions over all countries.") +
  theme_clean() +
  coord_cartesian(xlim = c(-0.005, 0.005)) +
