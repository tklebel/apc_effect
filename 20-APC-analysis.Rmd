...
papers_per_country <- works %>% 
  distinct(country, country_code, id, work_frac, author_position, institution_id) %>% # this is not correct yet, need to identify individual contributions correctly to count them correctly. might need to keep institution id as an identifier
...
average_apc <- works %>%
  ...
  distinct(id, work_frac, APC_in_dollar, University, country) %>% 
  group_by(country) %>%
